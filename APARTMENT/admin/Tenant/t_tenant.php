...
$results_per_page = 25; // Number of records per page
...
if ($selected_apartment_id) {
    $type_sql = "
      ...
      LEFT JOIN tenants t ON t.unit = h.unit
...
if ($selected_type_id) {
    $conditions[] = "house_types.id = $selected_type_id";
}
if (!empty($conditions)) {
    $query .= " WHERE " . implode(" AND ", $conditions);
}

$query .= " ORDER BY tenants.created_at DESC LIMIT $start_from, $results_per_page";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    $unit_result = mysqli_query($conn, "SELECT unit FROM tenants WHERE id = $delete_id");
    $unit_row = mysqli_fetch_assoc($unit_result);
    $unit = $unit_row['unit'];

    mysqli_query($conn, "DELETE FROM tenants WHERE id = $delete_id");
    mysqli_query($conn, "UPDATE houses SET status = 'Vacant' WHERE unit = '$unit'");
...
<aside class="sidebar" id="sidebar">
  ...
  <?php endif; ?>
      ...
      <?php if ($selected_apartment_id || $selected_type_id): ?>
        <a href="t_tenant.php" class="show-all-btn">🔄 Show All</a>
      <?php endif; ?>
...
<?php if (!$selected_apartment_id): ?>
  <div class="type-cards">
    <?php foreach ($apt_result as $apt): ?>
      <a href="t_tenant.php?apartment_id=<?= $apt['apartment_id'] ?>"
         class="type-card">
    ...
    <?php if ($selected_apartment_id && mysqli_num_rows($type_result) > 0): ?>
      <div class="type-cards">
        <?php while ($type = mysqli_fetch_assoc($type_result)): ?>
          <a href="t_tenant.php?apartment_id=<?= $selected_apartment_id ?>&type_id=<?= $type['id'] ?>" class="type-card <?= $selected_type_id == $type['id'] ? 'active' : '' ?>">
            <?= $type['type_name'] ?>
          ...
          <td>
            <div class="dropdown-container">
              ...
              <div class="dropdown-menu">
                <a href="edit_tenant.php?id=<?= $row['id'] ?>">Edit</a>
                <a href="payment_history.php?tenant_id=<?= $row['id'] ?>">Pay Partially</a>
                <form method="POST" onsubmit="return confirm('Are you sure you want to delete this tenant?');">
                  <input type="hidden" name="delete_id" value="<?= $row['id'] ?>">
                  <a href="prepayment.php?tenant_id=<?= $row['id'] ?>">Prepayment</a>
...
document.querySelectorAll('.dots-btn').forEach(btn => {
  btn.addEventListener('click', function (e) {
    ...
    document.querySelectorAll('.dropdown-menu').forEach(m => { if (m !== menu) m.classList.remove('show', 'open-up'); });
    menu.classList.toggle('show');
    const rect = menu.getBoundingClientRect();
    const spaceBelow = window.innerHeight - rect.bottom;
    const menuHeight = menu.offsetHeight;
    if (spaceBelow < menuHeight + 10) menu.classList.add('open-up');
  });
});
