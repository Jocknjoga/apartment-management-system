...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_id'])) {
    ...
    mysqli_query($conn, "DELETE FROM houses WHERE id = $delete_id");
...
if ($selected_apartment_id) {
    $type_sql = "
      SELECT 
        ...
        SUM(CASE WHEN h.status = 'Vacant' THEN 1 ELSE 0 END) AS vacant_units,
...
// Build main table query with filters
$params = [];
$sql = "
    SELECT h.id unit_id, h.unit, h.floor, h.status, h.rent, ht.type_name, a.apartment_name
    FROM houses h
...
$sql .= " ORDER BY h.unit LIMIT $offset, $limit";
$table_result = mysqli_query($conn, $sql);

// Determine which cards to show: apartments always; types only if apartment chosen
?>
...
<aside class="sidebar" id="sidebar">
  ...
  <?php endif; ?>
      <?php if ($selected_type_id || $selected_apartment_id): ?>
        <a href="housing.php" class="show-all-btn">🔄 Show All</a>
      <?php endif; ?>
  ...
  <div class="type-cards">
    <?php while ($type = mysqli_fetch_assoc($type_result)): ?>
      <a href="?apartment_id=<?= $selected_apartment_id ?>&type_id=<?= $type['id'] ?>"
        ...
        <p><strong>Total Units:</strong> <?= $type['total_units'] ?></p>
        <p style="color:green;"><strong>Vacant:</strong> <?= $type['vacant_units'] ?></p>
    </a>
    <?php endwhile; ?>
  </div>
<?php endif; ?>
    ...
    <section class="housing-table">
      <table class="table table-bordered" id="reportTable">
        ...
        <thead>
          <tr>
            <th>Apartment</th><th>Unit</th><th>Floor</th><th>Type</th><th>Status</th><th>Rent</th><th>Action</th>
          </tr>
        </thead>
        <tbody id="houseTable">
        <?php if ($table_result && mysqli_num_rows($table_result) > 0): ?>
          <?php while ($row = mysqli_fetch_assoc($table_result)): ?>
            <tr>
              ...
              <td style="color:<?= $row['status'] === 'Vacant' ? 'red' : 'green' ?>;">
                ...
                <?php if ($row['status'] === 'Vacant'): ?>
                  <div class="dropdown-container">
                    ...
                    <div class="dropdown-menu">
                      <a href="../Tenant/assign_tenant.php?unit_id=<?= $row['unit_id'] ?>">Assign</a>
                      <form method="POST" onsubmit="return confirm('Delete this house?');">
                        <input type="hidden" name="delete_id" value="<?= $row['unit_id'] ?>">
