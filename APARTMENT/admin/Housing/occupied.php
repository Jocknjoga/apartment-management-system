...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['unassign_id'])) {
    $house_id = intval($_POST['unassign_id']);

    // Get unit name from house ID
    $unit_query = mysqli_query($conn, "SELECT unit FROM houses WHERE id = $house_id");
    $unit_row = mysqli_fetch_assoc($unit_query);
    $unit = $unit_row['unit'];

    // Get tenant assigned to that unit
    $tenant_result = mysqli_query($conn, "SELECT * FROM tenants WHERE unit = '$unit'");
    if ($tenant_row = mysqli_fetch_assoc($tenant_result)) {
        
        // Archive the tenant to former_tenants table
        $archive_query = "INSERT INTO former_tenants 
          (name, national_id, phone, unit, apartment_name, moved_in_at, moved_out_at, status)
          VALUES (
            '{$tenant_row['name']}', 
            '{$tenant_row['national_id']}', 
            '{$tenant_row['phone']}', 
            '{$tenant_row['unit']}', 
            '{$tenant_row['apartment_name']}', 
        ...
        mysqli_query($conn, "DELETE FROM tenants WHERE unit = '$unit'");
    }

    // Set house status to 'Vacant'
    mysqli_query($conn, "UPDATE houses SET status = 'Vacant' WHERE id = $house_id");
...
// Fetch house types with count of occupied units for selected apartment
if ($selected_apartment_id) {
    $type_result = mysqli_query($conn, "
        SELECT ht.id, ht.type_name,
            COUNT(h.id) AS occupied_units
        FROM house_types ht
...
<aside class="sidebar" id="sidebar">
  ...
  <?php endif; ?>
      <?php if ($selected_type_id || $selected_apartment_id): ?>
        <a href="occupied.php" class="show-all-btn">🔄 Show All</a>
      <?php endif; ?>
  ...
  <div class="type-cards">
    <?php while ($type = mysqli_fetch_assoc($type_result)): ?>
      <a href="?apartment_id=<?= $selected_apartment_id ?>&type_id=<?= $type['id'] ?>"
        ...
        <p style="color:green;"><strong>Occupied:</strong> <?= $type['occupied_units'] ?></p>
    </a>
    <?php endwhile; ?>
  </div>
<?php endif; ?>
      ...
      <table class="table table-bordered" id="reportTable">
        ...
        <?php if ($table_result && mysqli_num_rows($table_result) > 0): ?>
          <?php while ($row = mysqli_fetch_assoc($table_result)): ?>
            ...
            <td>
              <form method="POST" onsubmit="return confirm('Unassign this house?');">
                  <input type="hidden" name="unassign_id" value="<?= $row['unit_id'] ?>">
                  <button type="submit" class="action-btn">Unassign</button>
                </form>
           </td>
